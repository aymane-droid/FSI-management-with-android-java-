...
public class DB_gestion  extends SQLiteOpenHelper {
    public DB_gestion(@Nullable Context context, @Nullable String name, @Nullable SQLiteDatabase.CursorFactory factory, int version) {
    ...
    public void onCreate(SQLiteDatabase db) {
            ...
            //db.execSQL("alter table Module ADD CONSTRAINT fk_child_parent FOREIGN KEY (CIN_P) REFERENCES Professeur(CIN) on delete cascade ");    //    " ,
            ...
            //db.execSQL("create table Inscription(CNE text,date text,titre text,primary key (CNE, titre)," +
                    //"constraint etu_fk foreign key (CNE) references Etudiant(CNE) ON DELETE CASCADE ON UPDATE CASCADE,\n" +
                    //"    constraint mod_fk foreign key (titre) references Module(titre) ON DELETE CASCADE ON UPDATE CASCADE)");
    ...
    void a(){
        ...
        SQLiteDatabase database=getWritableDatabase();
        database.execSQL("drop table Inscription");
    }
